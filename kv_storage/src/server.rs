...
impl Server {
    ...
    async fn handle_decided_entries(&mut self) {
        ...
        if self.last_decided_idx < new_decided_idx.try_into().unwrap() {
            let decided_entries = self.omni_paxos.read_decided_suffix(self.last_decided_idx.try_into().unwrap()).unwrap();
            self.update_database(decided_entries);
            self.last_decided_idx = new_decided_idx.try_into().unwrap();
            /*** reply client ***/
            let msg = Message::APIResponse(APIResponse::Decided(new_decided_idx.try_into().unwrap()));
            self.network.send(0, msg).await;
            // snapshotting
